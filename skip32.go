package skip32

/*
   SKIP32 -- 32 bit block cipher based on SKIPJACK.
   Written by Greg Rose, QUALCOMM Australia, 1999/04/27.

   In common: F-table, G-permutation, key schedule.
   Different: 24 round feistel structure.
   Based on:  Unoptimized test implementation of SKIPJACK algorithm
              Panu Rissanen <bande@lut.fi>

   SKIPJACK and KEA Algorithm Specifications
   Version 2.0
   29 May 1998

   Not copyright, no rights reserved.
*/

func Encrypt(key [10]uint8, input uint32) uint32 {
	return skip32(key, input, 0, true)
}

func Decrypt(key [10]uint8, input uint32) uint32 {
	return skip32(key, input, 23, false)
}

var ftable []uint8 = []uint8{
	,
}

func g(key [10]uint8, k, w uint32) uint32 {
	var g1, g2, g3, g4, g5, g6 uint32

	g1 = (w >> 8) & 0xff
	g2 = (w >> 0) & 0xff

	g3 = uint32(ftable[g2^uint32(key[(4*k+0)%10])]) ^ g1
	g4 = uint32(ftable[g3^uint32(key[(4*k+1)%10])]) ^ g2
	g5 = uint32(ftable[g4^uint32(key[(4*k+2)%10])]) ^ g3
	g6 = uint32(ftable[g5^uint32(key[(4*k+3)%10])]) ^ g4

	return (g5 << 8) + g6
}

func skip32(key [10]uint8, n, k uint32, encrypt bool) uint32 {
	var i, wl, wr uint32

	/* pack into words */
	wl = (((n >> 24) & 0xff) << 8) + (((n >> 16) & 0xff) << 0)
	wr = (((n >> 8) & 0xff) << 8) + (((n >> 0) & 0xff) << 0)

	/* 24 feistel rounds, doubled up */
	for i = 0; i < 24/2; i++ {
		wr ^= g(key, k, wl) ^ k
		if encrypt {
			k = k + 1
		} else {
			k = k - 1
		}
		
		wl ^= g(key, k, wr) ^ k
		if encrypt {
			k = k + 1
		} else {
			k = k - 1
		}
	}

	return wr << 16 | wl
}
